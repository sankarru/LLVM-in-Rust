...
fn build_eh_frame(text_size: u64, frame_size: u32, used_callee_saved: &[PReg]) -> Vec<u8> {
    ...
    if has_frame {
        ...
        for (idx, pr) in used_callee_saved.iter().enumerate() {
            ...
            // DWARF register number for this PReg (x86 enc direct mapping for callee-saved).
            // Mapping: PReg(3)=RBX→dwarf 3, PReg(12)=R12→dwarf 12, etc.
            // Our PReg numbering 0=RAX,1=RCX,2=RDX,3=RBX,4=RSP,5=RBP,6=RSI,7=RDI,8=R8..15=R15
